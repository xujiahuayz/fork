...
ax1.plot(
    end_times,
    emp_rate / hash_panel["fork_rate"],
    color="purple",
    label="$\\frac{" + freq_label + "}{\\text{historically measured fork rate}}$",
    linewidth=3,
    linestyle=":",
)
...
ax1.fill_between(
    ...
    emp_rate / (hash_panel["fork_rate"] + hash_panel["fork_rate_std"] * 1.645),
    color="purple",
    alpha=0.1,
    # label="90% confidence interval",
)
...
ax1.plot(
    end_times,
    emp_rate
    / (block_time_50 * hash_panel["total_hash_rate"] * (1 - hash_panel["hhi"]))
    / 100,
    label="$\\frac{"
    + freq_label
    + "}{\\Delta_0 \\cdot \\Lambda \\cdot (1-{\\it HHI})}$",
    color="olive",
    linewidth=3,
)
