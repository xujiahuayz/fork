...
for key, dict_items in {
    ...
    for block_propagation_time in set(list(EMPIRICAL_PROP_DELAY.values()) + [8.7]):
        ...
        plt.axhline(y=N_MINER, color="purple", linestyle="--", linewidth=0.5)
        plt.text(0.00075, N_MINER, f"$N={N_MINER}$", color="k")
        # vertical line at sigma=LOG_NORMAL_SIGMA
        plt.axvline(x=this_std, color="purple", linestyle="--", linewidth=0.5)
        ...
        # only show 3 ticks on the x-axis from 0 to 0.001
        plt.xticks(
            [0.0002, 0.0005, 0.0008],
            [f"{tick:.4f}" for tick in [0.0002, 0.0005, 0.0008]],
        )
        ...
        # add a horizontal line in the colorbar to indicate the fork rate
        cbar.ax.vlines(isoline_levels, ymin=0, ymax=1, colors="blue", linewidth=3)
        ...
        plt.savefig(
            FIGURES_FOLDER
            / f"fork_rate_heatmap_{key}_{round(block_propagation_time,4)}.pdf",
            bbox_inches="tight",
        )
        plt.show()
